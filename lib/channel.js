...
Channel.prototype.listen = function (latest) {
    ...
    this.latest(latest, this.handle(true, function (latest, collection) {
        try {
            var cursor = collection.find({ _id: { $gt: latest._id }}, {
                      tailable: true,
                      numberOfRetries: -1,
                      awaitdata: true,
        ...
        cursor.on('error', function(err){ self.emit('error', err); });
        // var more = function () {
        //     cursor.next(next);
        // };
        // more();
    ...
    this.collection.then(function (err, collection) {
        ...
        collection
            ...
            .next(function (err, doc) {
                ...
                //     if (err) return callback(err, null, collection);
                //     else if (docs && docs.length) return callback(err, docs[0], collection);
                // });
            });
    });
